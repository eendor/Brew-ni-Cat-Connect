...
**Document status:** Draft; the application toolchain is planned and has not yet been initialized.
...
The planned Phase 1 web foundation uses **Next.js with TypeScript**. Exact dependency versions and generated configuration will be recorded in `docs/software-and-apis.md` after initialization. This document does not claim that lint, formatting, test, type-check, build, or CI commands are currently available or have passed.
...
The exact structure may be refined by an architecture decision. The initial convention is:

```text
src/
  app/             Next.js routes, layouts, loading/error boundaries
  components/      Reusable presentation and interaction components
  features/        Feature-specific UI, actions, schemas, and orchestration
  domain/          Framework-independent business rules and value types
  lib/             Narrow infrastructure adapters and shared utilities
  styles/          Global design tokens/styles when needed
tests/             Cross-feature integration helpers/fixtures when justified
e2e/               Playwright specifications and fixtures
public/            Static, optimized public assets
...
- Do not import route/UI concerns into `domain`.
...
Phase 1 will configure automated tools and record pinned versions. The intended baseline is:
...
Planned local/CI scripts (names may be confirmed during Phase 1):
...
These are planned commands, not Phase 0 execution results.
...
During Phase 0, this definition establishes future gates; no application item is currently represented as done.
