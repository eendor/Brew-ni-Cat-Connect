...
**Date:** 2026-08-24\
**Status:** Accepted and implemented with a public-access blocker\
...
**Consequences:** Current public probes return HTTP 200 with zero rows while privileged read-only discovery confirms six categories and sixteen items; live public presentation remains In development until an approved minimum-field public view/policy exists. Phase 2 must not disable RLS, use a privileged key at runtime, mutate production, or present fake/poster prices. The adapter sorts categories/items alphabetically because no display-order field was discovered. Add-ons remain absent because the POS values are hardcoded rather than live catalog records. This narrow exception does not activate customer auth/order/backend scope early.
