...
| FR-003 | The system shall display configured location, Sunday-closed and variable-hours notice, contact information, payment/takeout facts, and social links. | Each confirmed field renders accurately and each link uses its configured destination; the site states that the shop is closed every Sunday and tells customers to check Facebook/contact the shop for other days rather than promising a fixed Mon–Sat schedule. | Must | 2 | Implemented |
