...
| FR-006 | The system shall present approved customer-favorite groups and shall obtain any displayed current product name or price from the authoritative menu source. | Matcha, Takoyaki, and Fries may be labelled as customer favorites without unsupported analytics; any specific catalog record/price matches the live source and a truthful state appears if public rows are unavailable. | Should | 2–4 | In development |
...
| ID | Requirement | Acceptance / verification | Priority | Target phase | Status |
| --- | --- | --- | --- | --- | --- |
| FR-010 | The system shall list publicly readable menu categories in authoritative order when supplied, with a documented deterministic fallback when the source has no display order. | Only categories returned by the approved public source appear; Phase 2 sorts valid names alphabetically because no display-order field was discovered, and shows a documented empty state when no public rows are readable. | Must | 2–4 | In development |
| FR-011 | The system shall list public products under their assigned categories using authoritative Supabase names and current prices. | Typed API/UI results match representative live records once anonymous read is approved; no poster-derived price appears, price formatting is consistent, and zero public rows produce a truthful empty state. | Must | 2–4 | In development |
...
| FR-015 | The system shall distinguish available, temporarily unavailable, and unknown-availability products without enabling purchase in Phase 2. | The public browse UI presents each returned availability state with text rather than color alone; Phase 3 ordering controls remain absent. | Must | 2–4 | In development |
...
| Authoritative menu, pricing, option, best-seller, promotion, and availability rules | FR-006–FR-016, FR-081 | Supabase is authoritative for current catalog fields; anonymous policy currently returns zero rows. Favorites groups are approved without exact rankings; promotions and ordering option rules remain **TODO: Confirm with Brew ni Cat owner.** |
...
The Phase 2 public routes and typed catalog client exist, and formatter, lint, type-check, unit/component, build, browser, documentation, responsive, accessibility, and security developer checks passed on 2026-08-24. **Implemented** does not mean independently Tested: Renier's QA and peer review remain pending.

FR-006, FR-010, FR-011, and FR-015 remain **In development** because the production anonymous role currently receives HTTP 200 with zero rows for both discovered catalog tables. The application handles that state honestly and never substitutes poster-derived or privileged data. Completing those requirements needs an approved least-privilege public-read policy/view and representative live-record verification; this phase does not change RLS or production data.
